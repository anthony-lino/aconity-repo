...
""" example usage for reading a .cli file """

if False:

    layers = readBinaryCli('lattice-updated-simp0p01-final-v21.cli')


    def plotLaserId(models, hatchGeom):
        buildStyle = pyslm.analysis.utils.getBuildStyleById(models, hatchGeom.mid, hatchGeom.bid)
        return buildStyle.laserPower


    (fig, ax) = pyslm.visualise.plot(layers[1], plot3D=False, plotOrderLine=False, plotArrows=False )

if False:

    filepath = 'modelsection_173_s2_vs.cli'
    layers = readCLI(filepath)
...
if True:
    ...
    """ example usage for writing a file """
    # Create a part from a mesh
    from pyslm import Part 
    solidPart = Part(name="example")
    solidPart.setGeometry('/CAD/examples/frameGuide.stl')
    ...
    # Create a model
    model = Model(mid=1, topSliceNum=10)
    ...
    pyslm.geometry.ModelValidator.validateBuild([model], [layer1])
    ...
    # Write ASCII Version
    writeCLI('filename.cli', header, [model], [layer1])

    # Write Binary Version
    #writeBinaryCLI('filename.cli', header, [model], [layer1])
